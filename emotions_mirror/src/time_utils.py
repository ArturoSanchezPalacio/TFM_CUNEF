...
def tdelta(input):
    ...
    regex = "".join(["\s*((?P<%s>\d+)\s*(%s(%s(s)?)?)(\s*(and|,)?\s*)?)?" % (k, k[0], k[1:-1]) for k in keys])
    kwargs = {}
    for k, v in re.match(regex, input).groupdict(default="0").items():
        kwargs[k] = int(v)
    return timedelta(**kwargs)

# This method simply expresses times in seconds as times in weeks, days, hours, minutes and seconds.
